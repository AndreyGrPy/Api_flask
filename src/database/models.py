...
class Film(db.Model):
    ...
    def __int__(self, title, release_date, description, distributed_by, length, rating, actors=None):
        self.title = title
...
class User(db.Model):
    ...
    uuid = db.Column(db.Text(33), unique=True)

    def __int__(self, username, email, password, is_admin=False):
